  ...
  if num_locations > 0:
    ...
    if assignment:
      ...
      for vehicle_nbr in range(num_vehicles):
        ...
        route_ft.append(node_index)
        route_ft.append(node_index_next)
        route_dist += dist_callback(node_index, node_index_next)
        dis_total+=route_dist
...
def create_data_cost():
  '''capacity=100    满载对比  零担 vs 整车 2:1
    如果整车设置为100/dis，那么零担就是2/dis*unit
  '''
  return 100,2
def plot(points,xy):
  x=[]
...
def simplePlan():
  [locations,demand]=create_data_array()
  start=0
  x=[]
  y=[]
  x.append(locations[start][0])
  y.append(locations[start][1])
  dis=0
  dist_between_locations = CreateDistanceCallback(locations)
  for i, j in enumerate(locations):
    if i==start:
      ...
      y.append(locations[start][1])
     # print(dis)
      dis+=dist_between_locations.Distance(start,i)*2
  plt.subplot('121')
  plt.plot(x,y,'r')
  print(dis)
  plt.title('Before Opt Total Dis'+str(dis))
  print('normal:',dis)
  #return dis

if __name__ == '__main__':
  simplePlan()
  route_opt,dis_opt=main()
  locations=create_data_array()[0]
  plt.subplot('122')
  plt.title('After Opt Total Dis'+str(dis_opt))
  plot(route_opt, locations)
  plt.show()